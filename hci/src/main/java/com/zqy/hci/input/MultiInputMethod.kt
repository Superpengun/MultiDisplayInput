...
class MultiInputMethod(
    ...
    override fun changeLanRes(lan: String) {
        ...
        InputEngineInstance.get().changeLanguage(lan)
    }
    ...
    override fun getNextPage() {
        if (checkComposingLength()) return
        val recogResult: RecogResult? = InputEngineInstance.get().multiGetMore()
    ...
    private fun checkComposingLength(): Boolean {
        return mComposingData.composingText.isEmpty()
    }

    private fun queryAndUpdateCandidate(query: String) {
        if (query.isEmpty()) return
        val recogResult: RecogResult = InputEngineInstance.get().multiQuery(query)
        val items: ArrayList<RecogResultItem>? = recogResult.getRecogResultItems()
        mCandidateWordsList.clear()
        synchronized(this){
            for (item in items!!) {
